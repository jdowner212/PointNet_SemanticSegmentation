...
def save_h5(h5_filename, data, label, data_dtype='uint8', label_dtype='uint8'):
  ### My own code:
    if os.path.exists(h5_filename):
        ...
        h5_fout["data"].resize((h5_fout["data"].shape[0] + data.shape[0]), axis=0)
        h5_fout["data"][-data.shape[0]:] = data

        h5_fout["label"].resize((h5_fout["label"].shape[0] + label.shape[0]), axis=0)
        h5_fout["label"][-label.shape[0]:] = label
        h5_fout.close()
    else:
